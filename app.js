/* ============================================================
   ...
   v10: Preferences card on the dashboard — preferred cities and
   expected salary, which personalize the Location and Salary
   scoring (previously neutral 80 for every job).
   v9: country-neutral rebrand. v8: interview prep. v7: cover
   letter generator. v6: PDF CV upload. v5: CV Analysis page.
   ============================================================ */
(function () {
  'use strict';
  ...
  /* ---------- scoring & filtering ---------- */
  function scoredJobs() {
    return DEMO_JOBS.map((job) => ({ job, res: Engine.scoreJob(state.profile, job) }));
  }
    ...
    return scoredJobs()
      .filter(({ job, res }) => {
        ...
        if (nl) {
          ...
          if (nl.minScore && res.score < nl.minScore) return false;
  ...
  const topMatches = (n) => scoredJobs().sort((a, b) => b.res.score - a.res.score).slice(0, n);
  ...
  function notifsView() {
    let h = topMatches(3).map(({ job, res }) =>
      '<div class="notif-item unread"><b>' + res.score + '% match</b> — ' + esc(job.title) +
  ...
  function homeView() {
    ...
    return '<section class="hero"><div class="container hero-grid"><div>' +
      ...
      step(2, 'Get scored matches', 'Every job is scored 0–100 across profession, qualifications, experience, licensing, skills, location and salary.') +
  ...
  function jobsView() {
    ...
    return '<section class="section"><div class="container">' +
      ...
      '<p class="muted" style="margin:4px 0 0">' + list.length + ' of ' + DEMO_JOBS.length + ' jobs · sorted by match score</p></div></div>' +
  ...
  function jobCard({ job, res }) {
    const pair = Engine.matchLabel(res.score);
    const color = pair[1];
    ...
    return '<div class="card hover job-card"><div class="job-top"><div>' +
      ...
      '<div class="ring-wrap">' + ring(res.score, color) + '<div class="ring-sub">' + matchBadge(res.score) + '</div></div></div>' +
      '<div class="job-meta">' +
  ...
  function dashboardView() {
    ...
    const good = scoredJobs().filter((x) => x.res.score >= 70).length;
    ...
    if (!savedIds.length) {
      ...
      savedIds.forEach((id) => {
        ...
        h += '<tr><td><b>' + esc(item.job.title) + '</b><br><span class="muted">' + esc(item.job.employer) + ' · ' + esc(item.job.city) + '</span></td>' +
          '<td>' + matchBadge(item.res.score) + '</td>' +
  ...
  function openJobModal(id) {
    ...
    $('#modal-root').innerHTML =
      ...
      '<button class="btn btn-ghost btn-sm" onclick="App.closeModal()">✕</button></div>' +
      '<div class="flex mt wrap">' + ring(res.score, color) + matchBadge(res.score) +
      salaryBadge(job) +
